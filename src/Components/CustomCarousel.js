    ...
    <div className="absolute top-0 flex items-center w-full h-128 box-border m-0 p-0 overflow-hidden shadow-xl z-40">
      {carouselImages.map((item, index) => {
        return (
          <div
            key={index}
            className="min-w-full h-128 duration-500"
      ...
      <button
        onClick={goLeft}
        className="absolute top-auto left-0 -translate-y-1/2 w-1/12 h-128"
      >
        O
      ...
      <button
        onClick={goRight}
        className="absolute top-auto right-0 -translate-y-1/2 w-1/12 h-128"
      >
        O
        <div className="absolute top-0 w-full h-full bg-black opacity-0 duration-200 hover:opacity-25" />
      </button>
